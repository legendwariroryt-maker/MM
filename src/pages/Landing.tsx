...
const Landing = () => {
  ...
  // Mouse parallax on the ambient orbs in the hero. We write directly to
  // CSS custom properties on :root — no React state, no per-frame React
  // re-render. The fixed-orbs <div> reads `var(--parallax-x)` / `var(--parallax-y)`
  // in a `transform: translate3d(...)` and the compositor handles the rest.
  const orbMouseX = useRef(0);
  const orbMouseY = useRef(0);
  ...
  // Nav transparency — toggle a class on <html> instead of triggering a
  // full React re-render on every scroll tick. The CSS rule lives at the
  // bottom of this file (`.nav-solid` selector).
  useEffect(() => {
    const html = document.documentElement;
    const handleScroll = () => {
      if (window.scrollY > 50) {
        if (!html.classList.contains("nav-solid")) html.classList.add("nav-solid");
      } else {
        if (html.classList.contains("nav-solid")) html.classList.remove("nav-solid");
      }
    };
    handleScroll();
  ...
  // Mouse parallax — captured at the section level for the hero orbs.
  // Writes CSS vars directly so React never re-renders on move/scroll.
  useEffect(() => {
    if (reduce) return;
    ...
    const flush = () => {
      pending = false;
      // Opposite-direction drift so the orbs feel "behind" the cursor.
      const x = -(orbMouseX.current * 18);
      const y = -(orbMouseY.current * 14);
      root.style.setProperty("--parallax-x", `${x}px`);
      root.style.setProperty("--parallax-y", `${y}px`);
    };
    const onMove = (e: PointerEvent) => {
      orbMouseX.current = (e.clientX / window.innerWidth) * 2 - 1;
      orbMouseY.current = (e.clientY / window.innerHeight) * 2 - 1;
      if (!pending) {
        pending = true;
  ...
  const heroWords = ["understood", "supported", "held", "seen"];
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-x-hidden font-sans text-foreground">
      ...
      {/* Ambient starfield — fixed behind everything */}
      <StarfieldBackground />

      {/* Ambient editorial background — soft blush & lavender + mouse parallax.
         The translate uses CSS variables written by the parallax effect above
         so React doesn't re-render on every move/scroll. */}
      <div
        aria-hidden
        className="pointer-events-none fixed inset-0 -z-20 overflow-hidden ambient-parallax"
      >
        ...
        <div
          className="absolute top-1/3 -right-40 w-[38rem] h-[38rem] rounded-full bg-accent/25 blur-[140px] animate-pulse-calm"
          style={{ animationDelay: "1.5s" }}
        />
        <div
          className="absolute bottom-0 left-1/3 w-[30rem] h-[30rem] rounded-full bg-secondary/40 blur-[120px] animate-pulse-calm"
      ...
      {/* Hero — editorial split with 3D sanctuary scene */}
      <section className="min-h-screen flex items-center relative px-6 pt-28 pb-20">
        <div className="max-w-7xl mx-auto w-full grid lg:grid-cols-[1.15fr_1fr] gap-14 items-center">
          ...
          >
            ...
            <h1 className="font-serif text-[3.25rem] md:text-[5rem] leading-[0.98] tracking-[-0.02em] text-foreground mb-8">
              ...
              <br />
              to feel{" "}
              <AnimatedHeadline words={heroWords} className="italic text-primary" />.
            </h1>
          ...
          >
            <div className="absolute -inset-6 rounded-[3rem] bg-gradient-to-br from-primary/20 via-accent/15 to-secondary/30 blur-3xl" />
            <div className="relative aspect-square w-full max-w-[520px] mx-auto rounded-[2.5rem] overflow-hidden">
              <HeroScene className="absolute inset-0" />
              {/* Floating "Vol. 01" tag */}
              <div className="absolute top-6 right-6 text-[10px] tracking-[0.32em] uppercase text-muted-foreground/80 bg-card/55 backdrop-blur-md rounded-full px-3 py-1 border border-border/60">
                Vol. 01
              </div>
              {/* Floating mascot chip — points to the Sir Hootington section */}
              <div className="absolute bottom-6 left-6 flex items-center gap-2 bg-card/65 backdrop-blur-md rounded-full pl-1 pr-4 py-1 border border-border/60 shadow-sm">
      ...
      <section className="py-24 px-6 relative">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14 scroll-animate opacity-0 translate-y-6">
            <span className="text-[10px] tracking-[0.36em] uppercase text-muted-foreground">
              Our promise
            </span>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {highlights.map((item, i) => (
              <TiltCard key={i} intensity={5}>
                <Card
                  className="scroll-animate opacity-0 translate-y-6 group p-10 glass-card border-border rounded-3xl hover:-translate-y-1 hover:shadow-medium"
                  style={{ transitionDelay: `${i * 100}ms` }}
                >
                  <div className="flex items-center gap-3 mb-6">
                    <span className="font-serif italic text-3xl text-primary/80">
                      0{i + 1}
                    </span>
      ...
      {/* Cinematic Video Showcase — tighter height so there's less dead
         scroll between it and the next section. 140vh = 1 viewport of
         visible video + ~0.4 viewport of "scroll past" before next section. */}
      <section
        ref={videoSectionRef}
        className="relative h-[140vh] px-4"
      >
          ...
          >
            ...
            >
              ...
              <h2 className="font-serif text-4xl md:text-6xl text-white leading-[1.05] drop-shadow-[0_4px_24px_rgba(0,0,0,0.6)]">
                Meet the friend who{" "}
                <span className="italic text-primary-foreground/90">listens</span>.
              </h2>
            ...
            <div className="absolute top-5 left-5 flex items-center gap-2 bg-black/25 backdrop-blur-md rounded-full px-3 py-1.5 border border-white/15">
              ...
              <span className="text-[10px] tracking-[0.24em] uppercase text-white/90">
                Sanctuary
              </span>
            </div>
          </motion.div>
          ...
          <div className="scroll-animate opacity-0 translate-y-6 grid md:grid-cols-[auto_1fr] items-center gap-14">
            <TiltCard intensity={4}>
              <div className="relative flex-shrink-0 mx-auto">
                ...
                <div className="relative w-72 h-72 md:w-96 md:h-96 rounded-full bg-card/60 border border-border flex items-center justify-center overflow-hidden">
                  <img
                    src={sirHootingtonStanding}
                    alt="Sir Hootington"
            ...
            <div>
              ...
              <div className="space-y-4 text-base md:text-lg text-muted-foreground leading-relaxed max-w-xl">
                <p>
                  Sir Hootington is no ordinary owl. A distinguished graduate of Harvard with a
                  double major in Psychology and "Advanced Listening Arts," he studied under the
                  legendary Owl of Minerva before devoting his life to helping teens navigate their
                  emotional journeys.
                </p>
                <p>
                  When he isn't dispensing wisdom, he sips chamomile tea, reorganises his collection
                  of self-help scrolls, and practises his signature "Supportive Head Tilt™" in the
                  mirror.
                </p>
      ...
      <section className="py-28 px-6 relative">
        <div className="max-w-4xl mx-auto">
          <TiltCard intensity={3}>
            <div className="scroll-animate opacity-0 translate-y-6 rounded-[2.5rem] glass-card border border-border p-10 md:p-16 shadow-medium">
              <div className="flex items-center gap-3 mb-6">
                <span className="h-px w-10 bg-foreground/40" />
                <span className="text-[10px] tracking-[0.36em] uppercase text-muted-foreground">
                  On MindfulMe
              ...
              <p className="font-serif text-2xl md:text-3xl leading-[1.4] text-foreground">
                Growing up isn't easy. Between school, friendships, and the quiet noise of your own
                thoughts, it's okay to feel overwhelmed. MindfulMe is a
                <span className="italic text-primary"> supportive friend</span> that listens —
                without judgement, without pressure, and always at your pace.
              </p>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-14 scroll-animate opacity-0 translate-y-6">
            ...
            <p className="text-muted-foreground max-w-sm">
              Eight quiet tools, thoughtfully designed for teenage minds — soft on the eyes, gentle
              on the heart.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            {features.map((feature, i) => (
              <TiltCard key={feature.section} intensity={5}>
                <Card
                  className="scroll-animate opacity-0 translate-y-6 group relative p-7 glass-card border-border rounded-3xl hover:-translate-y-1 hover:shadow-medium cursor-pointer overflow-hidden h-full"
      ...
      <section className="py-24 px-6">
        <div className="max-w-4xl mx-auto text-center scroll-animate opacity-0 translate-y-6">
          ...
          <blockquote className="font-serif italic text-3xl md:text-4xl text-foreground leading-[1.35]">
            "Your mental health journey is unique, and so is our support. No judgement, no pressure
            — just a caring companion ready to listen whenever you need."
          </blockquote>
          <div className="mt-8 flex items-center justify-center gap-3">
            <span className="h-px w-10 bg-border" />
            <span className="text-[10px] tracking-[0.36em] uppercase text-muted-foreground">
              Made for teens who matter
            </span>
            <span className="h-px w-10 bg-border" />
          </div>
        </div>
      ...
      <section className="py-28 px-6 relative overflow-hidden">
        <div className="max-w-4xl mx-auto scroll-animate opacity-0 translate-y-6">
          <TiltCard intensity={4}>
            <div className="relative rounded-[2.5rem] overflow-hidden border border-border glass-card p-12 md:p-20 text-center shadow-medium">
      ...
      {/* CSS for scroll animations + perf helpers */}
      <style>{`
        .scroll-animate {
        ...
        /* Ambient orbs follow CSS variables written by the parallax effect.
           The transform is on the compositor layer, so no React/JS work
           is required during scroll/move. */
        .ambient-parallax {
        ...
        /* Nav background swap — class-driven instead of state-driven so
           the scroll listener doesn't re-render React on every tick. */
        nav.fixed {
          background: transparent;
          transition: background-color 0.4s ease, backdrop-filter 0.4s ease,
            box-shadow 0.4s ease;
        }
        html.nav-solid nav.fixed {
        ...
        /* Reduce GPU cost: the editorial cards keep their glass look on
           hover but render flat during scroll to avoid backdrop-blur
           on every paint. */
        .glass-card {
